...
def test(model, sequence_length, test_mains, batch_size=512, gpu_dataset=False, pred_gate=None):
    if USE_CUDA:
        model = model.cuda()
    # Model test
    st = time.time()
...
class DM_GATE2(Disaggregator):
    def __init__(self, params):
        ...
        # self.train_window_shift = params.get('window_shift', 0)
    ...
    def partial_fit(self, train_main, train_appliances, pretrain=False, do_preprocessing=True,
        ...
        for appliance_name, power in train_appliances:
            ...
            if self.filter_train:
                ckpt_name = "./" + appliance_name + "_" + self.note + "_dm_g2_best_state_dict.pt"
            else:
                ckpt_name = "./" + appliance_name + "_" + self.note + "_dm_best_state_dict.pt"
