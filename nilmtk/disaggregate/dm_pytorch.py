...
def train(appliance_name, model, mains, appliance, epochs, batch_size, pretrain,
          checkpoint_interval=None, train_patience=3, gpu_dataset=True):
    # Model configuration
...
def test(model, test_mains, batch_size=512, gpu_dataset=True):
    ...
    test_dataset = GpuDataset(torch.from_numpy(test_mains).float().permute(0, 2, 1), gpu=gpu_dataset)
...
class DM(Disaggregator):
    ...
    def partial_fit(self, train_main, train_appliances, pretrain=False, do_preprocessing=True,
        ...
        print("Train main 2", train_main.shape)
    ...
    def normalize_data(self, data, sequence_length, mean, std,
        ...
        if overlapping:
            windowed_x = np.array([arr[i:i + n] for i in range(0, len(arr) - n + 1, overlapping_step)])
        else:
