...
from nilmtk.disaggregate.dm.forward import get_beta_schedule
...
class DDPM_Sampler(nn.Module):
    ...
    def posterior_params(self, x_t, t, noise_pred):
        ...
        alpha_one_minus_cumprod_sqrt_t = self.alphas_one_minus_cumprod_sqrt[t].view(x_t.shape[0],
                                                                                    1, 1)
        alpha_sqrt_recip_t = self.alphas_sqrt_recip[t].view(x_t.shape[0], 1, 1)

        mean = alpha_sqrt_recip_t * (x_t - beta_t * noise_pred / alpha_one_minus_cumprod_sqrt_t)
        std = self.betas_sqrt[t].view(x_t.shape[0], 1, 1)
